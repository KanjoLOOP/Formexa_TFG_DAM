...
class TestAuthArgon2(unittest.TestCase):
    ...
    def test_register_produces_argon2_hash(self):
        ...
        self.assertTrue(ok)
        row = self.db.fetch_one("SELECT password_hash FROM users WHERE username = ?", ("alice01",))
        self.assertIsNotNone(row)
        self.assertTrue(row[0].startswith("$argon2"), f"Expected argon2 hash, got: {row[0]}")
    ...
    def test_legacy_sha256_login_and_rehash(self):
        ...
        row = self.db.fetch_one("SELECT password_hash FROM users WHERE username = ?", ("legacy_user",))
        self.assertTrue(row[0].startswith("$argon2"), "Legacy hash was not rehashed to argon2")
    ...
    def test_salt_sha256_login_and_rehash(self):
        ...
        row = self.db.fetch_one("SELECT password_hash FROM users WHERE username = ?", ("salted_user",))
        self.assertTrue(row[0].startswith("$argon2"), "Salt+sha hash was not rehashed to argon2")
